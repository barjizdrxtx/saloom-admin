...
const MyDropdownReason = ({
  ...
  useEffect(() => {
    // Filter the dropdown items based on the search query for mobile number or name
    if (searchQuery.trim()) {
  ...
  const handleSelectOption = (value: any) => {
    ...
    setValue(value.id); // Update the setValue callback with the selected id
    setIsOpen(false);
  };
        ...
        <Fragment>
          ...
          <div className="bg-white w-full absolute z-50 mt-2 origin-top-right border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
            ...
            {filteredData?.map((el: any) => (
              ...
              >
                <div className="capitalize w-full py-2 pl-4">
                  {el[myvalue]} - {el.mobile}
                </div>
              </div>
            ))}
